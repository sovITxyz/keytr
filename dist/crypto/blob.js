...
export function serializeBlob(blob) {
    if (blob.iv.length !== IV_LENGTH)
        throw new BlobParseError(`IV must be ${IV_LENGTH} bytes`);
    if (blob.hkdfSalt.length !== SALT_LENGTH)
        throw new BlobParseError(`Salt must be ${SALT_LENGTH} bytes`);
    if (blob.ciphertext.length !== CT_LENGTH)
        throw new BlobParseError(`Ciphertext must be ${CT_LENGTH} bytes`);
